% CEG - Computational Evolutionary Biology
% Asgard Report
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.6]{talhouk_figure_1_part.png}
\end{center}
...
Analysis was performed using the Python {\tt Lifelines} package \cite{Davidson-Pilon2020}.  Metasamples ($H$ matrices) were calculated from the metagenes and expression matrix as explained in the following sub-section.  Metasample values were binariased to 0, 1 by thresholding at the median value.   Kaplan-Meier plots for made in respect of derived metasample for each application -- making $11 \times 3$ plots each with two survival curves (with 95\% confidence intervals).  A hazard ratio (HR)
