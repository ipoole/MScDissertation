...
\begin{tikzpicture}[node distance = 0.30cm, scale=1.3, every node/.style={scale=1.3}]
	...
	\node [left=\fudge of X] {\rotatebox{90}{\tiny{$n$ Genes}}};
...
\begin{tikzpicture}[node distance = 0.30cm, scale=1.3, every node/.style={scale=1.3}]
	...
	\node [left=\fudge of S] {\rotatebox{90}{\tiny{$nN$ Samples}}};
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[scale=0.4]{\plotspath{BoxAndScatters/genomic_feature_scatters_TCGA_AOCS.pdf}} 
...
From the clustered heatmap of figure \ref{fig-heatmap-BOTH} it is clear that the five components clustered to the left are highly sensitive to the batch effect, while in the other components batch is hardly discernable.  This is confirmed by the boxplots in figure \ref{fig-boxplots-BOTH}.

Table \ref{tab-integrative-analysis-BOTH} summarises observation on this combined dataset.  It can be seen that, with a few exceptions, a component is sensitive \emph{either} to Batch \emph{or} to some biological process, but seldom both.
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.7,draft=false]{\plotspath{SurvivalAnalysis/gene_specific_CD38_survival.pdf}}
\end{center}
