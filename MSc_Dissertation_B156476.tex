...
\footnote{Would it be better to state the selected ranks here?  As it is, this is left open until the results section}
...
Figure \ref{fig-AOCS-ica-fixed-vs-bootstrap} demonstrates that \emph{sampling} error is far greater then errors due to algorithm initialisation when the dataset is small (N=80), making metagene extraction highly unstable.  For brevity only results for ICA are shown, but a very similar effect is observable for PCA and NMF.   For this reason it was decided to perform all metagene extraction on the larger N=374 TCGA dataset, in which the impact of sampling error is not so severe -- see figure \ref{fig-TCGA-ica-fixed-vs-bootstrap}.  
...
High-level terms at depth less than 3 were removed, since these are uninteresting.   
...
\caption{Lineage maps of enriched Gene Ongology (GO) terms for components NMF-1-of-3 and NMF-2-of-3.  (NMF-3-of-3 produced no significant enrichment results)}
...
\item[HR Detect]: score from homologous repair (HR) analysis, deriving from Ewing \etal's work \cite{Ewing2020}
\item[Mutational Load]:
\item[CNV Load]: Copy-number variation ...
